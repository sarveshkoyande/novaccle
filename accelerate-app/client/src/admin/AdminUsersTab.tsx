...
const ROLE_TYPES = ['xm', 'cdm'];
const ROLE_LABEL: Record<string, string> = { xm: 'XM', cdm: 'CDM' };
...
const EMPTY: DraftUser = { name: '', email: '', roleType: ROLE_TYPES[0], brand: '' };

// The user directory behind the XM-to-brand / CDM-to-brand mapping — real
// named people, separate from the demo login personas (client/personas.ts).
// Both role types are mapped to exactly one brand (or none yet), per direct
// instruction, so "the mapping" is just this one dropdown per row, not a
// separate many-to-many screen.
export default function AdminUsersTab() {
  ...
  function openEdit(u: AppUser) {
    setDraft({ name: u.name, email: u.email || '', roleType: u.roleType, brand: u.brand || '' });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (!editing) return;
      const payload = { name: draft.name.trim(), email: draft.email.trim() || undefined, roleType: draft.roleType, brand: draft.brand || undefined };
    ...
    <div className="admin-layout" style={{ gridTemplateColumns: '1fr' }}>
      <section className="admin-field-panel">
        ...
        <table className="admin-ftable">
          <thead>
            <tr><th>Name</th><th>Role</th><th>Brand</th><th>Email</th><th></th></tr>
          </thead>
          <tbody>
            {users.length === 0 && (
              <tr><td colSpan={5} style={{ textAlign: 'center', color: 'var(--ink3)', padding: 24 }}>No users yet.</td></tr>
            )}
            {users.map((u) => (
